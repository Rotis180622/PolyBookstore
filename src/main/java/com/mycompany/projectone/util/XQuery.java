    ...
    private static <B> B readBean(ResultSet resultSet, Class<B> beanClass) throws Exception {
        ...
        for(Method method: methods){
            ...
            if (name.startsWith("set") && method.getParameterCount() == 1) {
                String columnName = null; //Lấy tên cột thực tế
                
                try {
                   Field field = beanClass.getDeclaredField(Character.toLowerCase(name.substring(3).charAt(0) ) + name.substring(4));
                   
                   if(field.isAnnotationPresent(Column.class)) {
                       columnName = field.getAnnotation(Column.class).name();
                   } else columnName = name.substring(3);
